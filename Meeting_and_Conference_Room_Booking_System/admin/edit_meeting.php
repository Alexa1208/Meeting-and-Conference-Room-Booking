...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $update = $pdo->prepare("UPDATE meeting_bookings 
                             SET room = ?, date = ?, start_time = ?, end_time = ?, purpose = ? 
                             WHERE id = ?");
    $update->execute([$room, $date, $start, $end, $purpose, $id]);
...
<head>
    <meta charset="UTF-8">
    <title>Edit Meeting Booking | Booking System</title>
...
<body>
<div class="container">
    <h3>Edit Meeting Booking</h3>

    <?php if (!empty($message)) echo "<div class='alert alert-success'>$message</div>"; ?>

    <form method="POST" class="card">
        <label>Room</label>
        <input type="text" name="room" value="<?= htmlspecialchars($booking['room']) ?>" class="form-control" required>
        ...
        <label>Purpose</label>
        <textarea name="purpose" class="form-control" required><?= htmlspecialchars($booking['purpose']) ?></textarea>

        <div class="button-row">
            <button type="submit" class="btn btn-primary">Update</button>
            <a href="meeting_list.php" class="btn btn-secondary">Cancel</a>
        </div>
    </form>
